...
$_group_list = filter_input(INPUT_POST, "GROUP_LIST");
...
$cats = explode("|", $_group_list);
$_group_id = "";
$_query = "";
foreach($cats as $cat) {
    $_group_id = (int) $cat;
    $_q = " GROUP_ID = ".$_group_id." OR ";
    $_query = $_query.$_q;
    $__pos->setGroupId($_group_id);
    if ($__pos->Insert() == 1) {}
}

$where      = "(".preg_replace('/\W\w+\s*(\W*)$/', '$1', $_query).")";
$QQQ        = "SELECT EMAIL, MOBILE_NO FROM VIEW_MEMBER_LIST WHERE ".$where." GROUP BY EMAIL, MOBILE_NO ";
$list       = $__View_Group_Members->loadAllByGroups($QQQ);
