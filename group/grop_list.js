 ...
 $(document).ready(loadTableSearch());
...
function loadTableSearch() {
    ...
    var s = $(this).val();
    var url = "../req_group/group_list_search.php?s="+s;
    $.ajax({
        type: "GET",
        url: url,
            ...
            if (parseInt(json.SUCCESS) === 1) {
                ...
                for (var i = 0; i < json.CONTENTS.length; i++) {
                    ...
                    + "<td class='type d-none d-md-table-cell'>" + json.CONTENTS[i].GROUP_TYPE + "</td>"
                                ...
                                + "<td class='last-modified d-none d-lg-table-cell'><a href=Group_Details.php?id=" + json.CONTENTS[i].ID + "' class='btn btn-success btn-sm'>View</a></td>"
                                + "<td class='d-table-cell d-xl-none'>"
                                + "<a class='btn btn-icon' href='Group_Details.php?id=" + json.CONTENTS[i].ID + "'>"
            ...
            if (parseInt(json.SUCCESS) === 1) {
                ...
                for (var i = 0; i < json.CONTENTS.length; i++) {
                    ...
                    + "<td class='type d-none d-md-table-cell'>" + json.CONTENTS[i].GROUP_TYPE + "</td>"
                                ...
                                + "<td class='last-modified d-none d-lg-table-cell'><a href=Group_Details.php?id=" + json.CONTENTS[i].ID + "' class='btn btn-success btn-sm'>View</a></td>"
                                + "<td class='d-table-cell d-xl-none'>"
                                + "<a class='btn btn-icon' href='Group_Details.php?id=" + parseInt(json.CONTENTS[i].ID) + "'>"
                                + "<i class='icon icon-information-outline'></i>"
                                + "</a>"
                                + "</td>"
                                + "</tr>";
